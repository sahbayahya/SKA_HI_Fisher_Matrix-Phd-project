import sys
sys.path.append("/Users/sahba/Dropbox/SKA_survey/SKA_HI_Fisher_Matrix/Fisher")
from CLtools import *
from scipy import linalg
from matplotlib import *
from pylab import *
import numpy as np
#reload(Fisher)

#==============# Functions===========================

def expplot(exp1, dx, dy, p, alpha, fill, lw):
	plotellsp(xo, yo, dx, dy, p, colors=expcolors[exp1], alpha=alpha, fill=fill, lw=lw)


def FoM(dx, dy, dxy, Delta_x):
	part1 = (dx**2 + dy**2)/ 2.0
	part2 =np.sqrt( (((dx**2 - dy**2 )**2)/ 4.0) + dxy**2)
	a = abs(part1 + part2)
	b = abs(part1 -  part2)
	FoM =np.pi/( np.pi * Delta_x * np.sqrt(a)* np.sqrt(b) )
	return FoM

#========================
#		Color selection
#========================
expcolors = {}
expcolors['WL'] = lightyellows
expcolors['SN'] =  reds
expcolors['BAO'] =lightblues
stage = 4
#=============================
#  Variables
#=============================

xvar, yvar = 'w0 wa'.split(' ')
xo, yo = 0, -1  # Best fit wa, wo

#========================================================================
# The Values of w_0 and w_a for a ll the sensitivites, 7.3 mJy For the SKA 
#========================================================================

Delta_x1 = 1.0
Delta_x2= 2.48

dx_150 =0.0109198717897  ; dy_150 =0.414272922179; dxdy_150 = 0.00315236530688 ; p_150 = dxdy_150/(dx_150 * dy_150)  #dxdy_100/dx_ * dy_7.3 #  100 mJy_diff_bins
dx_5 =   0.0036487       ; dy_5 = 0.0469348 ; dxdy_5 = 0.000100403273463; p_5 =  dxdy_5/(dx_5 * dy_5) # Values of w_0 , w_a and w_0w_a for  7.3 mJy_diff_bins
dx_Euclid= 0.0094467  ; dy_Euclid =0.114; dxdy_Euclid =  0.00099930672273; p_Euclid= dxdy_Euclid/(dx_Euclid* dy_Euclid)  # Euclid 14_diff_bins

#===========================================================
# Calculate the Figure of Merit as in Coe. arXiv: 0906.4123v1
#===========================================================
print
print "=========================================="
print "Figure of Merit using Coe DETF"
print "==========================================="
print 
fom_1mJy = FoM(dx_150, dy_150, dxdy_150, Delta_x1) ; print 'FoM  Coe DETF + SKA (150mJy) = ', fom_1mJy
fom_5 = FoM(dx_5, dy_5, dxdy_5, Delta_x1);           print 'FoM  Coe DETF + SKA ( 5 mJy) = ', fom_5
fom_Euclid = FoM(dx_Euclid, dy_Euclid, dxdy_Euclid,  Delta_x1) ; print 'FoM  Coe DETF + Euclid = ', fom_Euclid
print "=================Thanks!=========================="
print 
#====================# Plotting ====================
from matplotlib.pyplot import *
ax = subplot(1,1,1)
exp1= 'BAO'
p02 = expplot(exp1, dx_150, dy_150, p_150, alpha= 1, fill = 1, lw = 1)
exp3 = 'WL'
p03=  expplot(exp3, dx_Euclid, dy_Euclid, p_Euclid, alpha= 1, fill = 1, lw = 1)
exp2 = 'SN'
p01 =expplot(exp2, dx_5, dy_5, p_5, alpha= 1, fill = 1, lw = 1) 
ylabel('$w_0$', fontsize= 18)
xlabel('$\Omega_K$', fontsize= 18)
xmax = 0.03
xlim(-xmax,xmax)
ylim(-1.2, -0.7)
plot(xo, yo, 'kx')
plot(xo, yo, color = 'k', linestyle= '-',linewidth=1.5)
plot(xo, yo, color = 'k', linestyle= '-',linewidth=1.5)
savefig("plots/output_ellipse_w0_Ok.pdf")
print 'You should see a plot which with ellipses of different colors.'
print '(Looks like a big X at this scale.)'
print 'KILL PLOT WINDOW TO TERMINATE'
show()
