...
z = [] ; Da =[] ; hz =[]
(z_E, Da_E, H_E) = loadtxt('Outputs/output_EuclidmJy_ref.txt', unpack=True)
...
for i in range(len(plot_these_Srms)):
	...
	ax.plot(z[i], Da[i], color = colors_err[i],linewidth=1.5, linestyle="-", label=label_err[i])
	ax.scatter(z[i], Da[i], s= 50, marker='o',  edgecolor = colors_err[i], facecolor= colors_err[i])
p_E, = ax.plot(z_E,  Da_E , color = '#edd400',linewidth=1.5, linestyle="-")
p_BOSS, = ax.plot(z_BOSS, hz_BOSS, color= "#6E6E6E",linewidth =1.5,linestyle="-")

ax.scatter(z_E, Da_E, s= 50, marker='o',  edgecolor = '#edd400', facecolor= '#edd400')
ax.scatter(z_BOSS, hz_BOSS, s=50, marker='o', edgecolor= "#6E6E6E", facecolor="#6E6E6E")


l2 =legend([p_E, p_BOSS],[r"${\rm Euclid} \ {\rm }$", r"${\rm BOSS}$"], loc=4, frameon = False)
...
plt.ylim(0.1,  25)
plt.savefig('plots/output_lnda_mario_bias_added_Euclid.pdf')
show()
