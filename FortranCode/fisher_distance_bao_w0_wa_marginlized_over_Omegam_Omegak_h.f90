PROGRAM Fisher_Distance
  ...
  !====================================================================
!(100,250,500,750,1000,1500,2000,2500,3000,4000,5000,6000,7000,8000,9000,10000,12000,15000,17000,20000,22000,25000,27000,30000)
  !filename2 = 'file_numbr_list.txt'
...
! do k=1,24
 !   read(3, *, end=10) L 
  ...
  write(sarea,'(I5)') L
 ! ifile = ifile + 1
  filename = 'input_nz_SKA1_'//trim(adjustl(sarea))//'ref.txt'
  print*, filename
  open(2,file=filename,status='unknown')
  ...
  do ibin=1,100
    ...
    CALL report_result(z,bias,npara,fis,L)
    print*,''
    close(11)
...
CALL close_linearpk
 if(nbins>1)then
     ...
     write(12,*), 'N(z)=',Ntot
     !CALL report_result(z, bias,npara,fistot,L)
  endif
  print*,''
...
SUBROUTINE report_result(z,bias,npara,fis,L)
  IMPLICIT none
  ...
  open(12,file ='Fisher_diff_area_ska1_'//trim(adjustl(sarea))//'.txt', status='unknown') 
  ...
  !open(13,file='output_Fisher_bao_BOSS_S5000_bias.txt', status='unknown')
   open(13,file='output_diff_area_ska1_'//trim(adjustl(sarea))//'.txt', status='unknown')
